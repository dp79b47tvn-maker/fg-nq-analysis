...
<p>既然第20組整組反彈成正的違反直覺，就把這一組（SP500分數約 {dd_spx.get('top_score_range',[0,0])[0]:.0f}~{dd_spx.get('top_score_range',[0,0])[1]:.0f} 分，
n={dd_spx.get('top_n','—')}）單獨拉出來、再往下切細看內部。以下用<b>等數量分位（qcut）</b>呈現
（每組樣本數均勻、統計上較穩健），看未來20日超額報酬，樣本數少於10的子組用紅色標示（可信度低）：</p>
<div style="text-align:center;">{drilldown_img}</div>
<div class="callout">
  <b>拆細之後看到一個「倒U」形，原本整組的反彈其實藏著內部結構：</b>
  <ul>
    <li><b>從「偏貪婪」往「很貪婪」走，超額報酬是先往上爬的</b>（動能延續）：SP500大約在中段子組
    衝到最高（qcut最高那組約+0.53%），這段是撐起「第20組整組反彈」的主力。</li>
    <li><b>但真正最極端的貪婪（分數約92以上）就開始往下掉，不再是最高：</b>
    SP500最極端那個qcut子組（{dd_spx.get('qcut',[{}])[-1].get('label','—')} 分）超額報酬掉到 {dd_spx.get('qcut',[{}])[-1].get('excess',0):+.2f}%，直接翻負。</li>
    <li>換句話說：<b>「第20組整組反彈成正」這件事，主要是分數85~91這種「貪婪但還沒到極端」的區間貢獻的，
    不是真正最極端那一小撮</b>。真正最極端貪婪（92分以上）其實已經開始出現「退燒」的跡象，方向跟直覺一致，
    只是這一小撮的樣本較薄，統計上需要謹慎解讀，不能當成確定的結論。</li>
  </ul>
