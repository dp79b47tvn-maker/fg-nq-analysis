...
<div class="callout warn">
  ...
  幾乎全部集中在頭尾的極端讀數（尤其是最恐懼那端），中間的「中性」分數區間本身沒有清楚、可靠、
  能重複驗證的規律</b>——不管切法怎麼調，中間段從來沒有算出一個又強、又顯著、又能跨樣本／跨標的
  重現的方向。這對操作上的含意是：這個指數比較適合當「極端值警示器」（分數落到最低或最高的一小段
  區間時多留意），而不是拿來對整條0~100分做連續的「分數越低倉位越重」這種線性訊號——下一節的策略
  回測會直接驗證這個想法。
</div>
...
<div class="card">
  ...
  <p>
    ...
    只有最恐懼那一小段（最貪婪那段則因為動能延續效應，也不像單純的反向訊號）。
  </p>
  <p>
